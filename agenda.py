...
def adicionar_favoritos(agenda_favoritos, nome_favoritos, telefone_favoritos):
     dados_contato_favorito= {"nome_favorito": nome_favoritos, "telefone_favorito": telefone_favoritos}
     agenda_favoritos.append(dados_contato_favorito)
     print(f"\nO Contato de {nome_favoritos} foi adicionado aos favoritos com sucesso!")
     return

# Adicionar contato
def adicionar_contato(agenda, nome_contato, telefone_contato):
    dados_contato = {"nome": nome_contato, "telefone": telefone_contato}
...
def consultar_favoritos(agenda_favoritos):
     ...
     for contato in agenda_favoritos:
        print(f"{contato["nome_favorito"]} - {contato["telefone_favorito"]}")
     return
...
# Atualizar contatos
def atualizar_contato(agenda, nome_atual, novo_nome, novo_telefone):
    for contato in agenda:
        if ["nome"] == nome_atual:
...
def atualizar_favortitos(agenda_favoritos, nome_atual, novo_nome, novo_telefone):
    for contato in agenda_favoritos:
...
while True:
    ...
    if escolha == "1":
        ...
        if add_favoritos == "s":
            adicionar_favoritos(agenda_favoritos, nome, telefone)     
        ...
        if lista == "s":
            atualizar_favortitos(agenda_favoritos, nome_atual, novo_nome, novo_telefone)
        elif lista not in ["s", "n"]:
            print("\nResposta inválida!")  
        else:
            atualizar_contato(agenda, nome_atual, novo_nome, novo_telefone)  
